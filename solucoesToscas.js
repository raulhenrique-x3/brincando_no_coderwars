...
function createPhoneNumber(numbers){
  let numberCode = '';
  let middleNumberBody = '';
  let lastNumbers = '';
  
  const takingTheCode = numbers.slice(0,3);
  const numberCodeString = takingTheCode.join('');
  numberCode = '(' + numberCodeString + ') ';
  
  const takingTheMiddleNumberBody = numbers.slice(3, 6);
  const middleNumberBodyString = takingTheMiddleNumberBody.join('');
  middleNumberBody = middleNumberBody;
  
  const takingTheLastNumbers = numbers.slice(6, numbers.length);
  const lastNumbersString = takingTheLastNumbers.join('');
  lastNumbers = '-' + lastNumbersString;
  
  return numberCode + middleNumberBodyString + lastNumbers;
}
